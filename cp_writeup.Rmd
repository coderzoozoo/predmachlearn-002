...
Lets us train our model using Random Forest algorithm because it predicts the outcome with high accuracy. Since, the size of the training set is relatively smaller, Random Forest algorithm's drawback of lesser execution speed can be ignored. We should expect higher accuracy percentage than 95%, which means that out of sample error should be less than 5%.
...
```
The estimated accuracy is 96.9%. Estimated our of sample error is 3.1%, which is better than the expected out of sample error.
